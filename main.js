...
class PreviewModal extends obsidian.Modal {
	...
	async onOpen() {
		...
		if (this.prompt.variables.length > 0) {
			rp.createEl("h3", { text: "Variables" });
			var vc = rp.createDiv({ cls: "prompto-vars" });
			for (var vi = 0; vi < this.prompt.variables.length; vi++) {
				var vn = this.prompt.variables[vi];
				var g = vc.createDiv({ cls: "prompto-var-group" });
				g.createEl("label", { text: vn.replace(/_/g, " "), cls: "prompto-var-label" });
				var ta = new obsidian.TextAreaComponent(g);
				ta.setPlaceholder("Enter " + vn.replace(/_/g, " ") + "...");
				ta.setValue(this.variableValues[vn] || "");
				(function(name, textarea) { textarea.onChange(function(val) { self.variableValues[name] = val; }); })(vn, ta);
				ta.inputEl.addClass("prompto-var-input"); ta.inputEl.rows = 2;
			}
		}
		// Context
